...
let openERC2612Token, sigUtils, web3;
describe("TestOpenERC2612Token", function () {
    ...
    it("ERC2612Test", async function () {
        ...
        const domain = { name: 'ERC2612Token', version: '1', chainId:chainId, verifyingContract: openERC2612Token.address };
        const signature = await owner._signTypedData(domain, types, message);
        const { v, r, s } = yaoqi.ethers.utils.splitSignature(signature);
        console.log(v + "  " + r + "  " + s);
        await openERC2612Token.connect(other).permit(owner.address, other.address, 100, deadline, v, r, s);
